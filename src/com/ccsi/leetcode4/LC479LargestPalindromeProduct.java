...
public class LC479LargestPalindromeProduct {
    ...
    public static int largestPalindrome(int n){
        ...
        long half=upperBound*upperBound/(int)Math.pow(10,n); //利用最大的n位数找到可能组成回文数的前一半。

        boolean foundPalindrome=false;
        long palindrome=0;
        while(!foundPalindrome){
            //回文数从大到小试，先生产可能的回文数
            palindrome=Long.parseLong(half+new StringBuilder().append(half).reverse().toString());
            for (long i = upperBound; i*i >=palindrome ; i--) { //在upper和lower之间试，找能被整除的回文数
